...
import requests
from requests.exceptions import Timeout, ProxyError, SSLError, ConnectionError


def parse_twitter(query='', since='', language='en', days=1, popular=False, _name=''):
    class FakeTwitterError(Exception):
        pass

    def get_user_agents(check=True, request_url="", sleep_time=2):
        """
        Function make user agents list for scraping with opportunity to check them
        on specific site (HTML5 agents only for similar elements in html).
        Args:
        ...
        if check:
            for user_agent in user_agents:
                ...
                if req.status_code != 200 or not req.text.startswith("<!DOCTYPE html>"):
    ...
    def make_url(first=True, query='', since='', language='all', max_position=None, popular=False):
        """
        first: boolean, True if first request to search
        language: str with language from ISO 639-1 format, 'all' for all languages
        popular: boolean, True if search only popular tweets
        ...
        since: str, date in format '%Y-%m-%d'
        """
        FIRST_URL = 'https://twitter.com/search?'
        ...
        search = '&q='
        for count, arg in enumerate(query):
            operator = arg.replace(' ', '%20').replace(':', '%3A').replace('#', '%23').replace('@', '%40')
            if count == 0:
                search += '{}'.format(operator)
            else:
                search += '%20{}'.format(operator)
        if since != '':
            until = '%20until%3A{}'.format(
                datetime.strftime((datetime.strptime(since, "%Y-%m-%d") + timedelta(days=1)), "%Y-%m-%d"))
        ...
        search += (since + until)
        if first:
            url = '{}{}vertical=default{}{}&src=typd'.format(FIRST_URL, popular, search, language)
        else:
            url = '{}{}vertical=default{}{}&src=typd&include_available_features=1&include_entities=1&max_position={}&reset_error_state=false'.format(
                JSON_URL, popular, search, language, max_position)
        return url

    def tweet_download(url, headers, proxies, user_agent_list, proxies_list, max_position=None, first=True,
                       session=None):
        dummy = False
        fail = 0
        while True:
            try:
                if first:
                    ...
                    req = session.get(url, headers=headers, proxies=proxies, timeout=5)
                    tweets = BeautifulSoup(req.text, 'lxml').find_all("li",
                                                                      class_="js-stream-item stream-item stream-item ")
                    max_position = BeautifulSoup(req.text, 'lxml').find("div", class_="stream-container").attrs[
                        "data-min-position"]
                    headers['Referer'] = url
                    ...
                    req = session.get(url, headers=headers, proxies=proxies, timeout=5)
                    req = req.json()
                    if max_position == req["min_position"] and dummy:
                        tweets = False
                        break
                    if req['has_more_items'] == False and req['new_latent_count'] == 0:
                        # print(req)
                        url = re.sub(r'TWEET-.*(?=&)', req["min_position"], url)
                        max_position = req["min_position"]
                        dummy = True
                        raise FakeTwitterError
                    tweets = BeautifulSoup(req['items_html'], 'lxml').find_all("li",
                                                                               class_="js-stream-item stream-item stream-item ")
                    max_position = req["min_position"]
                break
            except (Timeout, ProxyError, SSLError, ConnectionError) as e:
                print("Error with connection. Changing proxy and UA.")
                ...
                pass
            except FakeTwitterError as e:
                print(e.__class__.__name__)
                pass
            except Exception as e:
                # Attribute и JsonRead ошибки
                print(e)
                print(url)
                if '<!DOCTYPE html>' in req.text and fail < 4:
                    print('fail')
                    session = requests.Session()
                    headers['User-Agent'] = choice(user_agent_list)
                    proxies["https"] = choice(proxies_list)
                ...
                pass
        return req, tweets, headers, proxies, user_agent_list, max_position, proxies_list, session
    ...
    def tweet_parse(tweet, number):
        ...
        """
        day[number] = {}
        name = tweet.find("span", class_="FullNameGroup")
        day[number]["name"] = name.find("strong").get_text(" ", strip=True)
        if name.find("span", class_="Icon Icon--verified"):
            day[number]["address_veriff"] = True
        else:
            day[number]["address_veriff"] = False
        if tweet.find("div", class_="ReplyingToContextBelowAuthor"):
            day[number]["is_reply"] = True
        else:
            day[number]["is_reply"] = False
        if tweet.find("div", class_="QuoteTweet-container"):
            day[number]["has_quote"] = True
        else:
            day[number]["has_quote"] = False
        day[number]["address"] = tweet.find("span", class_="username u-dir u-textTruncate").text
        day[number]["time"] = datetime.strftime(
            datetime.utcfromtimestamp(int(tweet.find("span", class_="_timestamp").attrs["data-time"])),
            "%Y-%m-%d %H:%M:%S")
        # text = ""
        # for i in tweet.find("p", class_='TweetTextSize').contents:
        #    if str(i).startswith("<img"):
        #        text += i.attrs["alt"]
        #    elif str(type(i)) == "<class 'bs4.element.NavigableString'>":
        #        text += str(i)
        #    else:
        #        text += i.text

        # day[number]["text"] = " ".join(text.replace('…', '...').replace(u'\xa0', u' ').replace('pic.twitter', ' pic.twitter').replace('http', ' http').strip().split())
        day[number]["text"] = " ".join(
            tweet.find("div", class_='js-tweet-text-container').text.replace('…', '...').replace(u'\xa0', u' ').replace(
                'pic.twitter', ' pic.twitter').replace('http', ' http').strip().split())
        day[number]["likes"] = int(tweet.find("span", "ProfileTweet-action--favorite u-hiddenVisually").find("span",
                                                                                                             "ProfileTweet-actionCount").attrs[
                                       "data-tweet-stat-count"])
        day[number]["retweets"] = int(tweet.find("span", "ProfileTweet-action--retweet u-hiddenVisually").find("span",
                                                                                                               "ProfileTweet-actionCount").attrs[
                                          "data-tweet-stat-count"])
        day[number]["replies"] = int(tweet.find("span", "ProfileTweet-action--reply u-hiddenVisually").find("span",
                                                                                                            "ProfileTweet-actionCount").attrs[
                                         "data-tweet-stat-count"])
    ...
    for _ in range(days):
        ...
        day = {}
        count = 0
        url = make_url(query=query, since=since, language=language, popular=popular)
        headers = {'User-Agent': choice(user_agent_list), "Accept-encoding": "gzip, deflate, br",
                   "Accept-language": "en-US,en;q=0.9"}
        proxies = {"https": choice(proxies_list)}
        session = requests.Session()
        req, tweets, headers, proxies, user_agent_list, max_position, proxies_list, session = tweet_download(url,
                                                                                                             headers,
                                                                                                             proxies,
                                                                                                             user_agent_list,
                                                                                                             proxies_list,
        ...
        while tweets:
            for tweet in tweets:
                day = tweet_parse(tweet, count)
                count += 1
            print("Download {} tweets".format(count), "on", day[count - 1]["time"])
            url = make_url(False, query, since, language, max_position, popular)
            sleep(round(uniform(1, 2.5), 2))
            req, tweets, headers, proxies, user_agent_list, max_position, proxies_list, session = tweet_download(url,
                                                                                                                 headers,
                                                                                                                 proxies,
                                                                                                                 user_agent_list,
        ...
        day.to_pickle("twitter_{}_{}_full.pkl".format(_name, since))
        print("Write {} tweets".format(count))
        since = until
        until = datetime.strftime((datetime.strptime(since, "%Y-%m-%d") + timedelta(days=1)), "%Y-%m-%d")
        sleep(round(uniform(5, 15), 2))
